...
//        stringStringKafkaProducer.send(new ProducerRecord<>("first", "atguigu"));
        // 发送一批数据
        for (int i = 0; i < 5; i++) {
            // 模式就是异步发送的,但是后面加个.get方法就变成同步发送拉。
            RecordMetadata first = stringStringKafkaProducer.send(new ProducerRecord<>("first", "atguigu" + i)).get();
            System.out.println("同步拿到返回结果，metadata是元数据的意思，主题:" + first.topic() + "分区:" + first.partition());
        }
        // 3. 关闭资源
